...
def Deroulement_tour_bot(Player_list, i, gui):
...
def Deroulement_tour_humain(Player_list, i, gui):
    Current_player = Player_list[i%2]
...
def main_pygame(Player_list, gui):
    i=0
    running = True
    while running:
        Current_player = Player_list[i%2]
        if Current_player.Type == "Humain":
            Flag, i = Deroulement_tour_humain(Player_list, i, gui)
            while Flag == 1:
                Flag, i = Deroulement_tour_humain(Player_list, i, gui)
        elif Current_player.Type == "AI":
            #time.sleep(0.5)
            Flag, i = Deroulement_tour_bot(Player_list, i, gui)
        if Flag == 0:
            running = False
        #print(Flag, i)
    return Player_list
